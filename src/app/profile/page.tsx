    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-8 animate-fade-in">
      ...
      <div>
        <div className="flex items-center gap-2">
          <h1 className="text-2xl font-black text-white">Account & Billing</h1>
          <span className="rounded-full bg-purple-500/10 px-2.5 py-0.5 text-xs font-bold text-purple-400">
            Plan Management
          </span>
        </div>
      ...
      <div className="brand-card p-6 sm:p-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-lg font-bold text-white">{activeBusiness.name}</h2>
              <span
          ...
          <div className="flex items-center gap-3">
            {activeBusiness.subscription_status === "trialing" && (
              <button
                ...
                className="flex items-center gap-1.5 rounded-xl border border-white/[0.08] bg-white/[0.03] px-4 py-2 text-xs font-bold text-white hover:bg-[#E8E8ED] transition-colors"
              >
        ...
        {activeBusiness.subscription_status === "trialing" && (
          <div className="mt-6 space-y-2 border-t border-white/[0.08] pt-5">
            <div className="flex items-center justify-between text-xs">
              <span className="font-semibold text-[#a1a1aa]">14‑Day Free Trial Progress</span>
              <span className="font-bold text-white">{trialDaysRemaining} / 14 Days Remaining</span>
            </div>
      ...
      <div className="space-y-6">
        <div className="text-center max-w-xl mx-auto space-y-2">
          <h2 className="text-xl font-black text-white">Upgrade Your Retention Engine</h2>
          <p className="text-xs text-[#a1a1aa]">
            Transparent, flat pricing with high return on investment. Cancel anytime.
          </p>

          {/* Billing Cycle Toggle */}
          <div className="inline-flex items-center gap-1 rounded-xl bg-white/[0.03] p-1 border border-white/[0.08] shadow-none mt-2">
            <button
              onClick={() => setBillingCycle("monthly")}
              className={`rounded-lg px-3 py-1.5 text-xs font-bold transition-all ${
                billingCycle === "monthly" ? "bg-purple-500 text-white" : "text-[#a1a1aa] hover:text-white"
              }`}
            >
              Monthly Billing
            </button>
            <button
              ...
              className={`rounded-lg px-3 py-1.5 text-xs font-bold transition-all ${
                billingCycle === "yearly" ? "bg-purple-500 text-white" : "text-[#a1a1aa] hover:text-white"
              }`}
            >
              Yearly (Save 20%)
            </button>
            ...
            return (
              <div
                ...
                className={`brand-card p-6 flex flex-col justify-between relative transition-all ${
                  plan.is_popular ? "border-purple-500 shadow-md ring-1 ring-[#a855f7]/20" : ""
                }`}
              >
                {plan.is_popular && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full brand-gradient text-white btn-interactive px-3 py-0.5 text-[10px] font-black text-white uppercase tracking-wider shadow-xs">
                    Most Popular
                  </div>
                )}
                ...
                <div>
                  ...
                  <p className="text-xs text-[#a1a1aa] mb-4">{plan.description}</p>

                  <div className="flex items-baseline gap-1 mb-6 pb-4 border-b border-white/[0.08]">
                  ...
                  <button
                    ...
                    className={`w-full py-3 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 cursor-pointer ${
                      isCurrent
                        ...
                        : plan.is_popular
                        ? "brand-gradient text-white btn-interactive text-white shadow-md shadow-[#a855f7]/20 hover:opacity-95"
                        : "bg-zinc-950 text-white hover:bg-[#1f235a]"
                    }`}
                  >
                    {isCurrent ? (
      ...
      <div className="brand-card p-6 sm:p-8 border-2 border-[#111439]/10 bg-gradient-to-br from-white to-[#F8F8F9] shadow-none">
        <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6">
          <div className="max-w-xl space-y-2">
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="text-lg font-black text-white">Founder / Super Admin Portal</h2>
              <span className="rounded-full bg-zinc-950/10 px-2.5 py-0.5 text-[10px] font-black uppercase tracking-wider text-white">
                Platform Owner
              </span>
            </div>
          ...
          <div className="w-full lg:w-96 bg-white/[0.03] p-5 rounded-2xl border border-white/[0.08] shadow-xs">
            {isFounderUnlocked || currentUser.role === "superadmin" ? (
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2 pt-2">
                  <Link
                    href="/admin"
                    className="flex-1 py-2.5 px-4 rounded-xl brand-gradient text-white btn-interactive text-white text-xs font-bold text-center shadow-none hover:opacity-95 flex items-center justify-center gap-2"
                  >
                  ...
                  <button
                    ...
                    className="py-2.5 px-3 rounded-xl border border-white/[0.08] bg-white/[0.03] text-[#a1a1aa] text-xs font-semibold hover:bg-gray-100"
              ...
              <form onSubmit={handleUnlockFounder} className="space-y-3">
                <div>
                  <label className="block text-[11px] font-bold text-white uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full rounded-xl border border-white/[0.08] bg-white/[0.03] pl-3.5 pr-10 py-2.5 text-xs text-white font-medium focus:outline-none focus:border-purple-500 focus:bg-white/[0.03]"
                      required
                    />
                    <button
                      type="button"
                  ...
                  <p className="text-[10px] text-[#a1a1aa] mt-1">
                    Master key: <code className="bg-gray-100 px-1 py-0.5 rounded text-white font-bold">FOUNDER2026</code>
                  </p>
                </div>

                <button
                  type="submit"
                  className="w-full py-2.5 rounded-xl bg-zinc-950 text-white text-xs font-bold hover:bg-[#1f235a] transition-all flex items-center justify-center gap-2 cursor-pointer shadow-none"
                >
      ...
      <div className="brand-card p-6">
        <div className="flex items-center gap-2 mb-4 border-b border-white/[0.08] pb-3">
          ...
          <h2 className="text-sm font-bold text-white">Billing History &amp; Invoices</h2>
        </div>
