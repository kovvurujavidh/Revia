...
export default function AdminPage() {
      ...
      <div className="min-h-[80vh] flex items-center justify-center p-4 animate-fade-in">
        <div className="max-w-md w-full brand-card p-8 text-center space-y-6">
          ...
          <div className="space-y-2">
            <h1 className="text-xl font-black text-white">Super Admin Access Restricted</h1>
          ...
          <div className="rounded-2xl bg-white/[0.03] p-4 text-left border border-white/[0.08] space-y-2">
            <div className="flex items-center justify-between text-xs font-bold text-white">
              ...
              <span className="capitalize px-2 py-0.5 rounded bg-gray-200 text-white text-[10px]">
                {currentUser.role}
              </span>
            </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 pt-2">
            <Link
              href="/dashboard"
              className="w-full flex items-center justify-center gap-2 rounded-xl bg-zinc-950 py-2.5 text-xs font-bold text-white hover:bg-[#1f235a] transition-colors"
            >
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-black text-white">Super Admin Control & Growth Planner</h1>
            <span className="rounded-full bg-red-500/10 px-2.5 py-0.5 text-xs font-bold text-red-400 flex items-center gap-1 border border-red-500/20">
        ...
        <div className="flex items-center gap-2">
          <Link
            href="/dashboard"
            className="flex items-center gap-1.5 rounded-xl border border-white/[0.08] bg-white/[0.03] px-3.5 py-2 text-xs font-bold text-white hover:bg-white/[0.03] transition-colors shadow-none"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl bg-zinc-950 px-3.5 py-2 text-xs font-bold text-white hover:bg-[#1f235a] transition-colors shadow-none"
      ...
      {/* Toast */}
      {toastMsg && (
        <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-xs font-bold text-emerald-400 flex items-center gap-2 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="brand-card p-5 border-l-4 border-l-[#a855f7]">
          ...
          <div className="flex items-baseline gap-2">
            <p className="text-3xl font-black text-white">₹{totalPlatformMRR.toLocaleString("en-IN")}</p>
            <span className="text-[10px] font-bold text-emerald-400 bg-emerald-500/10 px-1.5 py-0.5 rounded">
              +40% MoM
            </span>
          </div>
          <p className="text-[11px] text-[#a1a1aa] mt-1">ARR Run-Rate: ₹{totalProjectedARR.toLocaleString("en-IN")}</p>
        </div>

        <div className="brand-card p-5 border-l-4 border-l-[#10b981]">
          <div className="flex items-center justify-between mb-2 text-[#a1a1aa]">
          ...
          <div className="flex items-baseline gap-2">
            <p className="text-3xl font-black text-emerald-400">{activePaidBusinesses}</p>
            <span className="text-[11px] text-[#a1a1aa]">of {totalBusinesses} total</span>
          </div>
          <p className="text-[11px] text-[#a1a1aa] mt-1">Conversion: {Math.round((activePaidBusinesses / Math.max(1, totalBusinesses)) * 100)}%</p>
        </div>
        ...
        <div className="brand-card p-5 border-l-4 border-l-[#f59e0b]">
          ...
          <div className="flex items-baseline gap-2">
            <p className="text-3xl font-black text-amber-400">{trialBusinesses}</p>
            <span className="text-[10px] font-bold text-purple-400 bg-purple-500/10 px-1.5 py-0.5 rounded">
              14-Day Free
            </span>
          </div>
          <p className="text-[11px] text-[#a1a1aa] mt-1">High conversion pipeline</p>
        </div>
        ...
        <div className="brand-card p-5 border-l-4 border-l-[#ef4444]">
          ...
          <p className="text-3xl font-black text-red-400">{expiredBusinesses}</p>
          <p className="text-[11px] text-[#a1a1aa] mt-1">Win-back opportunities</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 brand-card p-6 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/[0.08] pb-4">
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base font-bold text-white">Platform MRR Growth Timeline</h2>
            ...
            <div className="flex items-center gap-1 rounded-xl bg-white/[0.03] p-1 border border-white/[0.08]">
              <button
                onClick={() => setTimeRange("6m")}
                className={`rounded-lg px-2.5 py-1 text-[11px] font-bold transition-colors ${
                  timeRange === "6m" ? "bg-white/[0.03] text-white shadow-none" : "text-[#a1a1aa]"
                }`}
              >
                6 Months
              </button>
              <button
                ...
                className={`rounded-lg px-2.5 py-1 text-[11px] font-bold transition-colors ${
                  timeRange === "1y" ? "bg-white/[0.03] text-white shadow-none" : "text-[#a1a1aa]"
                }`}
              >
                1 Year
              </button>
          ...
          <div className="h-56 w-full pt-4">
              ...
              {mrrGrowthData.map((item, index) => {
                ...
                return (
                  <div key={index} className="flex flex-col items-center h-full justify-end group relative">
                    {/* Tooltip */}
                    <div className="absolute -top-12 opacity-0 group-hover:opacity-100 transition-opacity bg-zinc-950 text-white rounded-lg p-1.5 text-[10px] font-bold pointer-events-none z-10 whitespace-nowrap shadow-lg">
                      <p>₹{item.mrr.toLocaleString("en-IN")}</p>
                      <p className="text-[9px] text-emerald-400">{item.growth} growth</p>
                    </div>

                    <div className="w-full flex items-baseline justify-center mb-1">
                      <span className="text-[10px] font-bold text-purple-400">{item.tenants}t</span>
                    </div>

                    <div
                      className="w-full rounded-t-xl brand-gradient text-white btn-interactive transition-all duration-500 group-hover:brightness-110 shadow-xs"
                      style={{ height: `${heightPct}%` }}
                    />

                    <p className="text-[10px] font-medium text-[#a1a1aa] mt-2 truncate max-w-full text-center">
                      {item.month.split(" ")[0]}
                    </p>
                  </div>
                );
            ...
            <div className="border-t border-white/[0.08] mt-1 pt-2 flex items-center justify-between text-[11px] text-[#a1a1aa] px-2">
              <span>SaaS Launch (Apr 2026)</span>
              <span className="font-semibold text-white">Current Run-Rate: ₹16,772/mo</span>
            </div>
          </div>
        </div>
        ...
        <div className="brand-card p-6 space-y-4">
          <div className="border-b border-white/[0.08] pb-3">
            <h2 className="text-base font-bold text-white">Tenant Industry Verticals</h2>
            <p className="text-xs text-[#a1a1aa]">Market share breakdown by business category</p>
          </div>
            ...
            {industriesList.map((item, index) => (
              <div key={index} className="space-y-1">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-semibold text-white capitalize">{item.industry}</span>
                  <span className="font-bold text-[#a1a1aa]">{item.count} stores ({item.percentage}%)</span>
                </div>
                <div className="w-full bg-[#E8E8ED] rounded-full h-2">
                  <div
                    className="h-2 rounded-full brand-gradient text-white btn-interactive transition-all"
          ...
          <div className="rounded-xl bg-white/[0.03] p-3 border border-white/[0.08] mt-4 text-[11px] text-[#a1a1aa] space-y-1">
      ...
      <div className="brand-card overflow-hidden">
        <div className="p-6 border-b border-white/[0.08] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-base font-bold text-white">Tenant Management Directory</h2>
            <p className="text-xs text-[#a1a1aa]">Manage subscriptions, extend trials, or view tenant workspace</p>
          </div>
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              className="w-full rounded-xl border border-white/[0.08] bg-white/[0.03] pl-9 pr-4 py-2 text-xs focus:outline-none focus:border-purple-500"
            />
          </div>
        </div>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-[#E8E8ED]">
              {filteredBusinesses.map((bus) => {
                ...
                return (
                  <tr key={bus.id} className={`hover:bg-white/[0.03]/50 transition-colors ${isSelected ? "bg-purple-500/5" : ""}`}>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="h-8 w-8 rounded-lg bg-zinc-950 text-white flex items-center justify-center font-bold text-xs">
                          {bus.name.charAt(0)}
                        </div>
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            {isSelected && (
                              <span className="rounded-md bg-purple-500 px-1.5 py-0.2 text-[9px] font-bold text-white uppercase">
                                Current
                              </span>
                            )}
                          </div>
                          <p className="text-[10px] text-[#a1a1aa] capitalize">{bus.industry} • {bus.phone}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 font-medium text-[#a1a1aa]">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="rounded-lg border border-white/[0.08] bg-white/[0.03] px-2.5 py-1 text-[11px] font-bold text-white hover:bg-gray-50 flex items-center gap-1 shadow-none"
                        ...
                        <button
                          ...
                          className="rounded-lg bg-zinc-950 px-2.5 py-1 text-[11px] font-bold text-white hover:bg-[#1f235a] flex items-center gap-1 shadow-none"
                        >
                          <span>Manage</span>
